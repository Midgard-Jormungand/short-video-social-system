...
object MyFollower extends Page {
  ...
  def addFollowState(id:Int):Unit={
    followerList.update{
      list=>list.map{
        i=>
          if(i.userId==id)
            FollowUserInfo(i.userId,i.name,i.signature,1)
          else
            i
      }
    }
  }
  def deleteFollowState(id:Int):Unit={
    followerList.update{
      list=>list.map{
        i=>
          if(i.userId==id)
            FollowUserInfo(i.userId,i.name,i.signature,0)
          else
            i
      }
    }
